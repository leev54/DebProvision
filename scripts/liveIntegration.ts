...
const fish=new FishClient(config.FISH_API_KEY,'https://api.fish.audio',config.FISH_HTTP_TIMEOUT_MS,config.FISH_TTS_MODEL,config.FISH_REALTIME_MODEL,config.FISH_REALTIME_TIMEOUT_MS);const smokeDb=openDatabase(dbPath(testDatabase)),cleanup=new ProviderDeletionQueue(smokeDb,fish);cleanup.start();
