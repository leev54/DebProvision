...
const fish=new FishClient(config.FISH_API_KEY,'https://api.fish.audio',config.FISH_HTTP_TIMEOUT_MS,config.FISH_TTS_MODEL,config.FISH_REALTIME_MODEL,config.FISH_REALTIME_TIMEOUT_MS);
const smokeDb=openDatabase(dbPath(testDatabase)),cleanup=new ProviderDeletionQueue(smokeDb,fish);cleanup.start();
let voiceId=process.env.FISH_TEST_VOICE_ID,created=false,primaryFailure:unknown;
try{
  const reference=process.env.FISH_TEST_REFERENCE_FILE;
  if(reference){const referenceAudio=await readFile(reference);const transcription=await new FishTranscriptionService(config.FISH_API_KEY,'https://api.fish.audio',config.FISH_ASR_TIMEOUT_MS).transcribeWav(referenceAudio);if(!transcription.text)throw new Error('Fish ASR smoke test returned no text');console.log(`Fish ASR returned ${transcription.text.length} characters.`);const model=await fish.createVoice({name:`deployment-smoke-${Date.now()}`,references:[{path:reference,...(process.env.FISH_TEST_REFERENCE_TEXT?{transcript:process.env.FISH_TEST_REFERENCE_TEXT}:{})}]});voiceId=model.id;created=true;console.log('Fish created a private temporary test model.');}
  if(!voiceId)throw new Error('Set FISH_TEST_VOICE_ID or FISH_TEST_REFERENCE_FILE to run real Fish TTS');
  const audio=await fish.synthesize({voiceId,text:'Deployment integration test successful.'});let liveBytes=0;await fish.streamSynthesize({voiceId,text:'Realtime integration test successful.'},chunk=>{liveBytes+=chunk.byteLength;});if(!liveBytes)throw new Error('Fish realtime smoke test returned no audio');console.log(`Fish realtime TTS completed with finish(stop) after ${liveBytes} audio bytes.`);const output=process.env.FISH_TEST_OUTPUT_FILE;if(output)await writeFile(output,audio);console.log(`Fish TTS returned ${audio.byteLength} bytes${output?` and wrote ${output}`:''}.`);
}catch(error){primaryFailure=error;
}finally{
  const failures:unknown[]=[];if(created&&voiceId)try{const deleted=await cleanup.delete(voiceId);if(deleted!=='deleted')failures.push(new Error('Temporary Fish model cleanup remains queued'));}catch(error){failures.push(error);}try{await cleanup.stopAndDrain();}catch(error){failures.push(error);}try{smokeDb.close();}catch(error){failures.push(error);}if(failures.length)primaryFailure=new AggregateError(primaryFailure?[primaryFailure,...failures]:failures,'Integration smoke cleanup failed');
}
