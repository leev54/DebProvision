...
interface Session {pipeline:LiveVoicePipeline;controller:AbortController;detach:()=>void;streams:Set<{destroy:(error?:Error)=>void}>;receiving?:symbol;groupId:string;switching:boolean;sequence:number;tail:Promise<void>}
export class DiscordLiveService {
  ...
  start(guild:string,connection:VoiceConnection,sourceUser:string,voiceId:string,alias=voiceId){
    ...
    const pipeline=new LiveVoicePipeline(sourceUser,voiceId,this.maxLagMs,alias);const session:Session={pipeline,controller:new AbortController(),streams:new Set(),groupId:`live:${guild}:${randomUUID()}`,switching:false,detach:()=>{},sequence:0,tail:Promise.resolve()};
    const handler=(user:string)=>{
      if(user!==sourceUser||session.controller.signal.aborted||session.receiving||session.switching)return;const token=Symbol('live-segment');session.receiving=token;
      const beganAt=Date.now(),sequence=session.sequence++,generation=session.groupId,targetId=session.pipeline.target();const opus=connection.receiver.subscribe(user,{end:{behavior:EndBehaviorType.AfterSilence,duration:350}});const decoder=new prism.opus.Decoder({rate:48_000,channels:2,frameSize:960});const chunks:Buffer[]=[];let bytes=0,completed=false,cleanEnding=false;
      const release=()=>{if(session.receiving===token)session.receiving=undefined;};const complete=(keep:boolean,error?:unknown)=>{if(completed)return;completed=true;release();clearTimeout(timer);session.streams.delete(control);opus.unpipe(decoder);if(!opus.destroyed)opus.destroy();if(!decoder.destroyed)decoder.destroy();if(error&&!session.controller.signal.aborted)logger.warn({err:error,guild},'live receive stream ended with an error');if(keep&&!session.controller.signal.aborted){const pcm=Buffer.concat(chunks),completedAt=Date.now();session.tail=session.tail.then(()=>this.process(guild,session,pcm,beganAt,completedAt,sequence,generation,targetId));}};
  ...
  private async process(guild:string,session:Session,pcm:Buffer,beganAt:number,completedAt:number,sequence:number,generation=session.groupId,targetId?:string){
    ...
    try{
      ...
      if(!result.text||!session.pipeline.active||session.controller.signal.aborted||session.groupId!==generation||remaining()<=0)return;
      timing.ttsQueuedAt=Date.now();const synthesisVoiceId=targetId??session.pipeline.target();
  ...
  async switchVoice(guild:string,id:string,alias?:string){
    ...
    if(session.pipeline.target()===id){session.pipeline.switchVoice(id,alias);return true;}
    session.switching=true;const oldGeneration=session.groupId;session.groupId=`live:${guild}:${randomUUID()}`;
    try{await this.cancelGroup(guild,oldGeneration);session.pipeline.switchVoice(id,alias);return true;}finally{session.switching=false;}
  }
}
