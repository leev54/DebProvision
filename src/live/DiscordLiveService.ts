...
export class DiscordLiveService {
  ...
  status(guild:string){return this.sessions.get(guild)?.pipeline;}async switchVoice(guild:string,id:string,alias?:string){const p=this.status(guild);if(!p)return false;await this.voice.stop(guild);p.switchVoice(id,alias);return true;}
}
