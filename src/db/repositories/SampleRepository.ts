...
export class SampleRepository {
  ...
  async remove(id:string){const sample=this.get(id);if(!sample)return;this.db.prepare('UPDATE samples SET active=0,is_best=0,selected=0 WHERE id=?').run(id);if(sample.filePath)await rm(sample.filePath,{force:true});this.db.transaction(()=>{this.db.prepare('DELETE FROM sample_reviews WHERE sample_id=?').run(id);this.db.prepare('DELETE FROM samples WHERE id=? AND active=0').run(id);})();}
  ...
  async purge(guild:string,owner?:string):Promise<PurgeResult>{const samples=this.list(guild,owner,false);const deactivate=this.db.prepare('UPDATE samples SET active=0,is_best=0,selected=0 WHERE id=?');this.db.transaction(()=>samples.forEach(sample=>deactivate.run(sample.id)))();const cleanupFailures:unknown[]=[];let deletedFiles=0;for(const sample of samples){try{if(sample.filePath)await rm(sample.filePath,{force:true});deletedFiles++;this.db.transaction(()=>{this.db.prepare('DELETE FROM sample_reviews WHERE sample_id=?').run(sample.id);this.db.prepare('DELETE FROM samples WHERE id=? AND active=0').run(sample.id);})();}catch(error){cleanupFailures.push(error);}}return {tombstoned:samples.length,deletedFiles,cleanupFailures};}
