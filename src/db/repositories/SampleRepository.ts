...
export class SampleRepository {
  ...
  async purge(guild:string,owner?:string){const samples=this.list(guild,owner,false);const deactivate=this.db.prepare('UPDATE samples SET active=0,is_best=0,selected=0 WHERE id=?');this.db.transaction(()=>samples.forEach(sample=>deactivate.run(sample.id)))();const failures:unknown[]=[];for(const sample of samples){try{if(sample.filePath)await rm(sample.filePath,{force:true});this.db.transaction(()=>{this.db.prepare('DELETE FROM sample_reviews WHERE sample_id=?').run(sample.id);this.db.prepare('DELETE FROM samples WHERE id=? AND active=0').run(sample.id);})();}catch(error){failures.push(error);}}if(failures.length)throw new AggregateError(failures,'One or more managed sample files could not be deleted');return samples.length;}
