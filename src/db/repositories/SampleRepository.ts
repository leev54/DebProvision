...
export class SampleRepository {
  ...
  async reserveStorage(bytes:number,limitBytes:number,root='/data/training'):Promise<()=>void>{
    ...
    try{
      ...
      this.reservedBytes+=bytes;let released=false;return ()=>{if(!released){released=true;this.reservedBytes-=bytes;}};
    }finally{unlock();}
  }
