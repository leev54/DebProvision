...
export class StorageCapacityError extends Error {constructor(){super('Training storage is full and no unprotected samples can be removed');this.name='StorageCapacityError';}}
...
export class SampleRepository {
  ...
  async remove(id:string){const sample=this.get(id);if(!sample)return;this.db.prepare('UPDATE samples SET active=0 WHERE id=?').run(id);if(sample.filePath)await rm(sample.filePath,{force:true});this.db.transaction(()=>{this.db.prepare('DELETE FROM sample_reviews WHERE sample_id=?').run(id);this.db.prepare('DELETE FROM samples WHERE id=?').run(id);})();}
  ...
  async purge(guild:string,owner?:string){const samples=this.list(guild,owner,false);this.db.transaction(()=>{const inactive=this.db.prepare('UPDATE samples SET active=0 WHERE id=?');for(const sample of samples)inactive.run(sample.id);})();for(const sample of samples)await this.remove(sample.id);return samples.length;}
  ...
  markBest(id:string){return this.db.transaction(()=>{const sample=this.db.prepare('SELECT guild_id,voice_owner_id FROM samples WHERE id=? AND active=1 AND review_status=?').get(id,'pending') as {guild_id:string;voice_owner_id:string}|undefined;if(!sample)return 0;this.db.prepare("UPDATE samples SET is_best=0 WHERE guild_id=? AND voice_owner_id=? AND review_status='pending'").run(sample.guild_id,sample.voice_owner_id);return this.db.prepare("UPDATE samples SET is_best=1 WHERE id=? AND active=1 AND review_status='pending'").run(id).changes;})();}
  async reserveStorage(bytes:number,limitBytes:number,root='/data/training'):Promise<StorageReservation>{
    ...
    try{
      ...
      if(used+this.reservedBytes+bytes>limitBytes)throw new StorageCapacityError();
      this.reservedBytes+=bytes;let settled=false;const settle=()=>{if(!settled){settled=true;this.reservedBytes-=bytes;unlock();}};return {commit:settle,release:settle};
