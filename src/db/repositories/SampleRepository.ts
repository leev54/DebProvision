...
export class SampleRepository {
  ...
  async reconcileManagedStorage(root='/data/training'){
    const managed=path.resolve(root);await mkdir(managed,{recursive:true});const rows=this.db.prepare('SELECT id,file_path,active FROM samples').all() as {id:string;file_path:string;active:number}[];const referenced=new Set<string>();const missing:string[]=[];
    for(const row of rows){const file=path.resolve(row.file_path);const relative=path.relative(managed,file);const parts=relative.split(path.sep);const generated=parts.length===3&&/^\d{15,22}$/.test(parts[0]!)&&/^\d{15,22}$/.test(parts[1]!)&&/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.wav$/i.test(parts[2]!);if(!generated)continue;if(row.active){referenced.add(file);if(await access(file).then(()=>false,()=>true))missing.push(row.id);}}
    ...
    const guilds=await readdir(managed,{withFileTypes:true});let removed=0;for(const guild of guilds){if(!guild.isDirectory()||!/^\d{15,22}$/.test(guild.name))continue;const guildPath=path.join(managed,guild.name);for(const owner of await readdir(guildPath,{withFileTypes:true})){if(!owner.isDirectory()||!/^\d{15,22}$/.test(owner.name))continue;const ownerPath=path.join(guildPath,owner.name);for(const entry of await readdir(ownerPath,{withFileTypes:true})){if(!entry.isFile()||!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.wav$/i.test(entry.name))continue;const file=path.join(ownerPath,entry.name);if(!referenced.has(file)){await rm(file,{force:true});removed++;}}}}
