...
export class SampleRepository {
  ...
  async reserveStorage(bytes:number,limitBytes:number,root='/data/training'){
    ...
    const diskUsage=async(dir:string):Promise<number>=>{const entries=await readdir(dir,{withFileTypes:true}).catch(()=>[]);return (await Promise.all(entries.map(entry=>entry.isDirectory()?diskUsage(`${dir}/${entry.name}`):stat(`${dir}/${entry.name}`).then(x=>x.size).catch(()=>0)))).reduce((a,b)=>a+b,0);};let used=await diskUsage(root);
    for(const x of sizes){if(used+bytes<=limitBytes)break;if(x.r.review_status==='accepted'||x.r.selected)continue;await this.deactivate([x.r.id]);used-=x.size;}
    if(used+bytes>limitBytes)throw new Error(`Training storage limit reached (${Math.round(limitBytes/1024/1024)} MB); accepted samples were preserved`);
  }
  private map(r:any):ScoredSample{return {id:r.id,ownerId:r.voice_owner_id,filePath:r.file_path,transcript:r.transcript??undefined,durationMs:r.duration_ms,capturedAt:r.captured_at,qualityScore:r.quality_score,speechRatio:r.speech_ratio,silenceRatio:1-r.speech_ratio,rms:r.rms,peak:r.peak,clippingRatio:r.clipping_ratio,snrEstimate:r.snr_estimate,reasons:JSON.parse(r.reasons),isBestSample:!!r.is_best,exceptionalCandidate:!!r.exceptional,selectedForRebuild:!!r.selected,reviewStatus:r.review_status,active:!!r.active};}
}
