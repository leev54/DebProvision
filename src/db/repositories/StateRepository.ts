...
export class StateRepository {
  ...
  operationalError(){return (this.db.prepare('SELECT last_error FROM application_state WHERE id=1').get() as {last_error:string|null}).last_error;}
  recordOperationalError(message:string){this.db.prepare('UPDATE application_state SET last_error=? WHERE id=1').run(message.slice(0,500));}
