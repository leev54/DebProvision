...
export function openDatabase(file:string){
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS voices(id INTEGER PRIMARY KEY,guild_id TEXT NOT NULL,owner_id TEXT NOT NULL,alias TEXT NOT NULL COLLATE NOCASE,fish_voice_id TEXT,model_version INTEGER NOT NULL DEFAULT 0,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL,last_rebuilt_at INTEGER,UNIQUE(guild_id,owner_id),UNIQUE(guild_id,alias));${samplesSchema.replace('CREATE TABLE','CREATE TABLE IF NOT EXISTS')};CREATE TABLE IF NOT EXISTS sample_reviews(id INTEGER PRIMARY KEY, sample_id TEXT NOT NULL, reviewer_id TEXT NOT NULL, decision TEXT NOT NULL, created_at INTEGER NOT NULL);CREATE TABLE IF NOT EXISTS generations(id TEXT PRIMARY KEY,guild_id TEXT NOT NULL,voice_id INTEGER NOT NULL,requester_id TEXT NOT NULL,text_length INTEGER NOT NULL,created_at INTEGER NOT NULL,status TEXT NOT NULL);`);
