...
describe('Discord user-id command UX',()=>{
 it('requires one command channel and administrator in the configured guild',()=>{const config=loadConfig({DISCORD_TOKEN:'x',DISCORD_CLIENT_ID:'123456789012346',DISCORD_GUILD_ID:'123456789012345',ADMIN_USER_ID:'234567890123456',BOT_COMMAND_CHANNEL_ID:'345678901234567',FISH_API_KEY:'x',DATABASE_URL:'file:/tmp/x'} as NodeJS.ProcessEnv);expect(isConfiguredGuild(config.DISCORD_GUILD_ID,config.DISCORD_GUILD_ID)).toBe(true);expect(config.BOT_COMMAND_CHANNEL_ID).toBe('345678901234567');expect(()=>loadConfig({DISCORD_TOKEN:'x',DISCORD_CLIENT_ID:'123456789012346',DISCORD_GUILD_ID:'123456789012345',FISH_API_KEY:'x',DATABASE_URL:'file:/tmp/x'} as NodeJS.ProcessEnv)).toThrow();expect(()=>loadConfig({...config,BACKUP_DIRECTORY:'/data/training/backups'} as any)).toThrow()});
