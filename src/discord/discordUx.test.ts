...
describe('Discord user-id command UX',()=>{
 ...
 it('registers controls without live',()=>{const names=commands.map(x=>x.name);expect(names).toEqual(expect.arrayContaining(['pause','start','diagnostics','play']));expect(names).not.toContain('live')});
