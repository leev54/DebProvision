...
export class DatasetCurator {
  ...
  prune(samples:ScoredSample[],maxCount:number){const keep=this.select(samples,maxCount,Number.MAX_SAFE_INTEGER);return {kept:keep,removed:samples.filter(s=>!keep.includes(s)&&s.reviewStatus!=='accepted'&&!s.selectedForRebuild&&!s.isBestSample)};}
}
