...
export class DatasetCurator {
  constructor(private pendingMinScore=.78){}
  select(samples:ScoredSample[],maxCount=200,maxSeconds=600){
    ...
    const rest=eligible.filter(s=>s.reviewStatus==='pending'&&!s.selectedForRebuild&&s.qualityScore>=this.pendingMinScore).sort((a,b)=>b.qualityScore-a.qualityScore);
  ...
  prune(samples:ScoredSample[],maxCount:number){const keep=this.select(samples,maxCount,Number.MAX_SAFE_INTEGER);return {kept:keep,removed:samples.filter(s=>!keep.includes(s)&&(s.reviewStatus==='rejected'||s.reviewStatus!=='accepted'&&!s.selectedForRebuild&&!s.isBestSample))};}
}
