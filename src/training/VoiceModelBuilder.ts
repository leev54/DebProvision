...
export class VoiceModelBuilder {
  constructor(private provider:VoiceProvider,private curator=new DatasetCurator(),private maxCount=200,private maxSeconds=600){}
  async build(name:string,samples:ScoredSample[],validate:(id:string)=>Promise<void>){
    const selected=this.curator.select(samples,this.maxCount,this.maxSeconds).filter(s=>s.filePath&&s.reviewStatus!=='rejected');
