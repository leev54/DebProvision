...
async function main(){
  ...
  const repo=new VoiceRepository(db),samples=new SampleRepository(db),state=new StateRepository(db),readiness=new ModelReadinessRepository(db);repo.reconcileStaged();await samples.cleanupQueuedFiles();await samples.reconcileManagedStorage();
  const temp=new TempFiles();await temp.init();const fish=new FishClient(config.FISH_API_KEY,'https://api.fish.audio',config.FISH_HTTP_TIMEOUT_MS,config.FISH_TTS_MODEL),providerDeletions=new ProviderDeletionQueue(db,fish),localCleanup=new ManagedStorageCleanup(samples),voice=new DiscordVoiceRuntime(),capture=new VoiceCaptureService('/data/training',undefined,undefined,config.MAX_TRAINING_SEGMENT_SECONDS*1000,bytes=>samples.reserveStorage(bytes,config.MAX_TRAINING_STORAGE_MB*1024*1024),config.MIN_TRAINING_SEGMENT_SECONDS*1000),training=new TrainingSessionManager(),builder=new VoiceModelBuilder(fish,id=>providerDeletions.delete(id),new DatasetCurator(config.MODEL_SAMPLE_MIN_SCORE),config.FISH_MAX_MODEL_REFERENCES,config.MAX_SELECTED_TRAINING_DURATION_SECONDS,id=>repo.stageProvider(id),id=>repo.clearStaged(id),'/data/training'),backups=new BackupService(db,state,config.BACKUP_DIRECTORY,config.BACKUP_RETENTION_COUNT),client=createDiscordClient(config,repo,{samples,fish,voice,capture,training,builder,providerDeletions,state,readiness,db,startedAt},false);
  cleanup=async()=>{if(closed)return;closed=true;await backups.stopAndDrain();await shutdownRuntime({drainInteractions:()=>drainDiscordInteractions(client),stopCapture:()=>capture.stopAllAndDrain(),stopLive:async()=>{},stopVoice:()=>voice.stopAll(),stopLocalCleanup:()=>localCleanup.stopAndDrain(),stopProviderCleanup:()=>providerDeletions.stopAndDrain(),client,db});};
