...
export class VoiceCaptureService {
  private listeners=new Map<string,()=>void>();private activeUsers=new Map<string,Set<string>>();private streams=new Map<string,Map<string,Set<Finalizer>>>();private pending=new Map<string,Map<string,Set<Promise<void>>>>();
  constructor(private root='/data/training',private extractor=new SampleFeatureExtractor(),private scorer=new SampleQualityScorer(),private maxSegmentMs=30_000,private reserveStorage?:(bytes:number)=>Promise<(()=>void)|void>,private minSegmentMs=2_000){}
  start(guild:string,connection:VoiceConnection,users:string[],onSample:(sample:ScoredSample)=>Promise<boolean|void>|boolean|void){
    if(this.listeners.has(guild))throw new Error('Capture is already running');this.activeUsers.set(guild,new Set(users));this.streams.set(guild,new Map());this.pending.set(guild,new Map(users.map(user=>[user,new Set()])));
