...
export class VoiceCaptureService {
  ...
  start(guild:string,connection:VoiceConnection,users:string[],onSample:(sample:ScoredSample)=>Promise<boolean|void>|boolean|void){
    ...
    const begin=(userId:string)=>{if(this.stoppingGuilds.has(guild)||!this.activeUsers.get(guild)?.has(userId))return;const userStreams=this.streams.get(guild)?.get(userId)??new Set<Finalizer>();this.streams.get(guild)?.set(userId,userStreams);if(userStreams.size)return;
      const opus=connection.receiver.subscribe(userId,{end:{behavior:EndBehaviorType.AfterSilence,duration:1000}});const decoder=new prism.opus.Decoder({rate:48_000,channels:2,frameSize:960});const chunks:Buffer[]=[];let finalized=false;
      ...
      const timer=setTimeout(()=>{userStreams.delete(control);finalize();if(!this.stoppingGuilds.has(guild)&&this.activeUsers.get(guild)?.has(userId))setImmediate(()=>begin(userId));},this.maxSegmentMs);decoder.on('data',(chunk:Buffer)=>chunks.push(Buffer.from(chunk)));
      let completionStarted=false;const complete=()=>{if(completionStarted)return;completionStarted=true;finalized=true;clearTimeout(timer);opus.unpipe(decoder);if(!opus.destroyed)opus.destroy();void this.finish(guild,userId,chunks,onSample).finally(()=>{userStreams.delete(control);decoder.destroy();pending.delete(completion);completed();});};
