...
export class DiscordVoiceRuntime {
  ...
  enqueueBytes(guildId:string,label:string,bytes:Uint8Array,_format:'mp3'|'wav'='mp3'){const state=this.guilds.get(guildId);if(!state)throw new Error('Join a voice channel first');const id=randomUUID();state.queue.enqueue({id,label,play:async signal=>{const player=createAudioPlayer({behaviors:{noSubscriber:NoSubscriberBehavior.Play}});const resource=createAudioResource(Readable.from(bytes),{inputType:StreamType.Arbitrary});state.connection.subscribe(player);signal.addEventListener('abort',()=>player.stop(true),{once:true});player.play(resource);await entersState(player,AudioPlayerStatus.Playing,10_000);await new Promise<void>((resolve,reject)=>{player.once(AudioPlayerStatus.Idle,resolve);player.once('error',reject);signal.addEventListener('abort',()=>resolve(),{once:true});});}});return id;}
  enqueueStream(guildId:string,label:string,produce:(stream:PassThrough,signal:AbortSignal)=>Promise<void>){const state=this.guilds.get(guildId);if(!state)throw new Error('Join a voice channel first');const id=randomUUID();state.queue.enqueue({id,label,play:async signal=>{const stream=new PassThrough({highWaterMark:64*1024});const player=createAudioPlayer({behaviors:{noSubscriber:NoSubscriberBehavior.Play}});state.connection.subscribe(player);signal.addEventListener('abort',()=>{stream.destroy();player.stop(true);},{once:true});player.play(createAudioResource(stream,{inputType:StreamType.Arbitrary}));const generated=produce(stream,signal).then(()=>stream.end(),error=>stream.destroy(error));await new Promise<void>((resolve,reject)=>{player.once(AudioPlayerStatus.Idle,resolve);player.once('error',reject);signal.addEventListener('abort',()=>resolve(),{once:true});});await generated;}});return id;}
