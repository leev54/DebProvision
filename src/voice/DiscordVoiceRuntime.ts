...
export async function produceIntoStream(stream:PassThrough,signal:AbortSignal,produce:(stream:PassThrough,signal:AbortSignal)=>Promise<void>){try{await produce(stream,signal);stream.end();}catch(error){stream.destroy(error as Error);throw error;}}
