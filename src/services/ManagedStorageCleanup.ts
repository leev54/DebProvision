...
export class ManagedStorageCleanup {
  ...
  run(){if(this.stopping||this.inFlight)return this.inFlight??Promise.resolve();this.inFlight=this.samples.reconcileManagedStorage(this.root).then(()=>{},error=>logger.error({err:error},'managed storage reconciliation failed')).finally(()=>{this.inFlight=undefined});return this.inFlight;}
