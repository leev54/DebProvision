...
export class ProviderDeletionQueue {
  ...
  enqueue(remoteId:string){const now=this.now();this.db.prepare('INSERT INTO pending_provider_deletions(remote_id,next_retry_at,created_at,retryable) VALUES(?,?,?,1) ON CONFLICT(remote_id) DO NOTHING').run(remoteId,now,now);}
