...
export class ProviderDeletionQueue {
  ...
  async delete(remoteId:string){this.enqueue(remoteId);return this.stopping?'retry_scheduled':this.attemptReady(remoteId);}
  attempt(remoteId:string){return this.attemptReady(remoteId);}
  armAndAttempt(remoteId:string){this.arm(remoteId);return this.attemptReady(remoteId);}
  attemptReady(remoteId:string){if(this.stopping){this.enqueue(remoteId);return Promise.resolve('retry_scheduled' as const);}const row=this.db.prepare('SELECT ready_at,created_at FROM pending_provider_deletions WHERE remote_id=?').get(remoteId) as {ready_at:number|null,created_at:number}|undefined;if(!row||!row.ready_at&&row.created_at>this.now()-ProviderDeletionQueue.CRASH_RECOVERY_HOLD_MS)return Promise.resolve('retry_scheduled' as const);const existing=this.flights.get(remoteId);if(existing)return existing;const work=this.perform(remoteId).finally(()=>this.flights.delete(remoteId));this.flights.set(remoteId,work);this.track(work);return work;}
  ...
  retryDue(){if(this.stopping)return Promise.resolve();if(this.retryRun)return this.retryRun;const run=(async()=>{const now=this.now();const rows=this.db.prepare('SELECT remote_id FROM pending_provider_deletions WHERE retryable=1 AND next_retry_at<=? AND (ready_at IS NOT NULL OR created_at<=?) ORDER BY next_retry_at').all(now,now-ProviderDeletionQueue.CRASH_RECOVERY_HOLD_MS) as {remote_id:string}[];for(const row of rows)await this.attemptReady(row.remote_id);})();this.retryRun=run.finally(()=>{this.retryRun=undefined;});this.track(this.retryRun);return this.retryRun;}
