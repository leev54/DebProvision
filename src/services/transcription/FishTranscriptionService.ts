...
export class FishTranscriptionService implements TranscriptionService {
  ...
  async transcribeWav(audio:Uint8Array):Promise<Transcription>{
    const form=new FormData();
    form.set('audio',new Blob([Buffer.from(audio)],{type:'audio/wav'}),'speech.wav');
    form.set('ignore_timestamps','true');
    const response=await fetch(`${this.base}/v1/asr`,{method:'POST',headers:{Authorization:`Bearer ${this.apiKey}`},body:form,signal:AbortSignal.timeout(this.timeoutMs)});
