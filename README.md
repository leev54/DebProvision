...
Training and playback commands are `/train start|stop|status|rebuild|rebuild-all`, `/trainingdata status|clear|prune`, `/bestsample show|accept|reject|history`, `/say`, `/play query:<song, artist, or YouTube URL>`, `/voices`, `/voiceinfo`, `/join`, `/leave`, `/stop`, `/skip`, and `/queue`.
...
- `/start` clears that persisted state. Only `/start` and read-only `/diagnostics` are accepted while paused.
...
Samples retain the existing score, review state, Fish reference-count limit, and selected-duration rules. As qualifying samples arrive, DebProvision fingerprints the eligible dataset and marks the model **rebuild ready** in `/trainingdata status`. A successful existing rebuild records that exact fingerprint. Readiness compares the deterministic count-, duration-, curator-, and byte-bounded outbound reference plan with the exact IDs used by the last successful build (including any HTTP 413 reduction). Unselected overflow does not create false readiness. When ready, `/trainingdata status` provides a one-click **Rebuild now** button. Rebuild execution remains explicit so provider staging, replacement coordination, and durable deletion guarantees are preserved.

## Music playback

Music is resolved through an isolated `yt-dlp` adapter, streamed through the shared playback queue, limited by `MAX_MUSIC_DURATION_SECONDS`, and controlled by the existing `/skip`, `/stop`, and `/queue` commands. Music is never stored in training storage or backups.
