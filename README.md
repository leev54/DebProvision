...
### Fish TTS model
`FISH_TTS_MODEL` selects the HTTP model and `FISH_REALTIME_MODEL` independently selects the WebSocket model. Both accept `s1`, `s2-pro`, `s2.1-pro`, or `s2.1-pro-free` under Fish Audio's current canonical contracts.
